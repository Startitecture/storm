namespace SAF.ProcessEngine
{
    ...
    /// <summary>
    /// Encapsulates a state changed event condition that should trigger execution of a command.
    /// </summary>
    /// <typeparam name="T">The type that represents the possible states of the trigger.</typeparam>
    public class StateChangedCommandTrigger<T> : CommandTrigger<StateChangedEventArgs<T>>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="StateChangedCommandTrigger&lt;T&gt;"/> class with the 
        /// specified command, sender, and state.
        /// </summary>
        /// <param name="command">The command to execute.</param>
        /// <param name="sender">The sender of the event.</param>
        /// <param name="state">The state to trigger on.</param>
        public StateChangedCommandTrigger(IExecutable command, IStateMachine<T> sender, T state)
        ...
        public StateChangedCommandTrigger(
            IExecutable command, 
            IStateMachine<T> sender,
            T state,
            Predicate<StateChangedEventArgs<T>> additionalCondition)
        ...
        private static bool CheckBaseCondition(
            object source, StateChangedEventArgs<T> e, IStateMachine<T> sender, T state)
        {
