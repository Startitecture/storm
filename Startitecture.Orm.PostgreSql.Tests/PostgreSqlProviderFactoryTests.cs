    ...
    public class PostgreSqlProviderFactoryTests
    {
        ...
        private static IConfigurationRoot ConfigurationRoot =>
            new ConfigurationBuilder().AddJsonFile("appsettings.json", false)
                .AddUserSecrets<PostgreSqlProviderFactoryTests>(false)
                .Build();
        ...
        public void Create_PostgreSqlProvider_CountOfAggregateOptionTypeGreaterThanZero()
        {
            ...
            {
                var actual = provider.DatabaseContext.ExecuteScalar<int>("SELECT COUNT(1) FROM dbo.\"AggregateOptionType\"");
        ...
        [TestMethod]
        [TestCategory("Integration")]
        public async Task CreateAsync_PostgreSqlProvider_CountOfAggregateOptionTypeGreaterThanZero()
        {
            var target = new PostgreSqlProviderFactory(new DataAnnotationsDefinitionProvider());
        ...
        public void DeleteUnitTestItems()
        {
            ...
            using (var provider = providerFactory.Create(ConfigurationRoot.GetConnectionString("OrmTestDbPg")))
            {
                ...
                provider.Delete(Query.From<AggregateOptionRow>().Where(set => set.AreEqual(row => row.Name, "UNIT_TEST:%")));
                provider.Delete(Query.From<OtherAggregateRow>().Where(set => set.AreEqual(row => row.Name, "UNIT_TEST:%")));
                provider.Delete(Query.From<DomainAggregateRow>().Where(set => set.AreEqual(row => row.Name, "UNIT_TEST:%")));
                provider.Delete(Query.From<SubContainerRow>().Where(set => set.AreEqual(row => row.Name, "UNIT_TEST:%")));
                provider.Delete(Query.From<TopContainerRow>().Where(set => set.AreEqual(row => row.Name, "UNIT_TEST:%")));
                provider.Delete(Query.From<CategoryAttributeRow>().Where(set => set.AreEqual(row => row.Name, "UNIT_TEST:%")));
                provider.Delete(Query.From<TemplateRow>().Where(set => set.AreEqual(row => row.Name, "UNIT_TEST:%")));
                provider.Delete(Query.From<DomainIdentityRow>().Where(set => set.AreEqual(row => row.UniqueIdentifier, "UNIT_TEST:%")));
            }
        }
    }
}