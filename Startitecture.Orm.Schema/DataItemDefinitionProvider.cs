...
namespace Startitecture.Orm.Schema
{
    ...
    public class DataItemDefinitionProvider : IEntityDefinitionProvider
    {
        ...
        private static IEnumerable<PropertyInfo> GetFilteredEntityProperties(Type entityType, params Type[] attributeTypes)
        {
            ...
            {
                excludedProperties.AddRange(Enumerable.Select(typeof(ITransactionContext).GetNonIndexedProperties(), PropertyNameSelector));
            }

            if (typeof(ICompositeEntity).IsAssignableFrom(entityType))
            {
                excludedProperties.AddRange(Enumerable.Select(typeof(ICompositeEntity).GetNonIndexedProperties(), PropertyNameSelector));
            }
            ...
            foreach (var propertyInfo in properties)
            {
                ...
                {
                    var customAttributes = CustomAttributeExtensions.GetCustomAttributes((MemberInfo)propertyInfo);
        ...
        private static EntityLocation GetEntityNameByType(EntityReference entityReference)
        {
            ...
            var sourceTableNameAttribute = CustomAttributeExtensions.GetCustomAttributes<TableNameAttribute>((MemberInfo)entityType, true).FirstOrDefault();
